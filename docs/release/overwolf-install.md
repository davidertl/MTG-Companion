...
2. **Overwolf web app** — `apps/overwolf/` (Setup + Main windows).
...
3. Load in Overwolf (development / sideload):
   ...
   - In Overwolf, use **Load unpacked extension** (wording may vary by Overwolf version) and point at the folder that contains `manifest.json`, `main.html`, `setup.html`, and `mancutg-arenac.exe`.

4. Launch MTG Arena (optional for UI; required for log activity). Open the app from Overwolf; complete **Setup** (Player.log path), then **Main**.

## User-facing notes

- **Player.log** is usually under:
  `%USERPROFILE%\AppData\LocalLow\Wizards Of The Coast\MTG Arena\Player.log`
- The **Setup** window calls `GET /v1/detect-player-log` on the sidecar to prefill this path when possible.
- **Live watcher** in the Main window polls `POST /v1/watch/tick` every 2 seconds while running.
